...
If needed, install the package. The latest version (v0.3.0) includes the segment building functionality we are presenting here.
```{r}
...
In this example we want to add another rule to help define the scope of our segment.  The next rule limits the visits to be on January 25, 2022.  
```{r}
...
>**Note:** There are some quirks to the API that we are still trying to figure out.  One of these is represented in this example.  While the UI is clearly a list of mobile device types such as "Mobile Phone", the API is actually needing a number. That is why we are using the numeric "eq" verb with a dimension. I only include this example to point out the fact that this process of building segments using code is valuable but there is a learning curve. Most of the time you will not have any problems at all but there are some that may require you to do a bit of research to get it right using the API.

## Create the Container
Every segment has at least one container. Adding an additional container to the main container is only needed if you are using a different "conjunction" between then. Here we are creating a "visit" context container using "rule1" and "rule2" and using the conjunction "and". This will result in a segment that includes visits where both rules are **true**.
...
In the following example the segment name is "Home Page visits" and the description is "All visits that include a page named 'Home'".  We are using the "containers" argument because we have at least one container to be included in our segment. If we were just using rules then we would use the "rules" argument. 
